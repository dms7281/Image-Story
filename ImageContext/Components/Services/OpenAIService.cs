...
{
    private readonly OpenAI.OpenAIClient _api = new(configuration["OpenAIKey"]);
    ...
    public async Task<string> AddressCorrection(List<string?> originalAddresses, byte[] imageBytes)
    {
        ...
        var chat = _api.GetChatClient("gpt-4o");
        //chat.RequestParameters.TopP = 0.05;

        var response = await chat.CompleteChatAsync ("You will be given an image of a location and a address that is near the location, pictured in the photo, or IS the location pictured in the photo." +
                                 ...
                                 ChatMessage.CreateSystemMessage(ChatMessageContentPart.CreateImagePart(BinaryData.FromBytes(imageBytes), "image", ChatImageDetailLevel.Low)));        
    ...
    public async Task<string> HistoryOfAddress(string? address)
    {
        ...
        var response = await chat.CompleteChatAsync("You will be given an address, summarize the history of this address. " +
    ...
    public async Task<string> CreateOptimalSearchTerm(string? address)
    {
        ...
        var response = await chat.CompleteChatAsync("You will be given an address, pick the part of the address that would be most ideal as a search term. " +
