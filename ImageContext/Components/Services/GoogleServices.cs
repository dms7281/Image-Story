...
public class GoogleServices(IHttpClientFactory httpClientFactory, IConfiguration configuration)
{
    ...
    public async Task<(List<string?>, List<string?>)> GeocodingApi (string lat, string lng)
    {
    ...
    public async Task<List<WebResult>> SearchApi(List<string?> searchTerms, DateTime date)
    {
        ...
        // {
            ...
            var request = await httpClient.GetAsync($"v1?cx={_googleSearchEngine}&q={searchTerms[0]}+after:{afterDate}+before:{beforeDate}&key={_key}");
